...
else
{
    setcookie('GUEST_BOOK_POS',GetCurUrl(),time()+3600,'/');
     ...
     $sql = 'select    dede_archives.* , dede_addon18.*   from   dede_addon18  left join   dede_archives  on  dede_archives.id= dede_addon18.aid    ' ;
    
 
    $dlist = new DataListCP();
    $dlist->pageSize = 100;
 
    $dlist->SetTemplate(DEDETEMPLATE.'/plus/booking_room_con0.htm');
    $dlist->SetSource($sql);
    $dlist->Display();
}